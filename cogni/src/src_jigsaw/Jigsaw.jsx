...
class Jigsaw extends React.Component {
  ...
  class YourComponent extends Component {
    ...
    const mainContentNode = ReactDOM.findDOMNode(this.mainContentRef.current);
    const contentBoxNode = ReactDOM.findDOMNode(this.contentBoxRef.current);

    if (mainContentNode && contentBoxNode) {
    const dynamicPadding = (mainContentNode.getBoundingClientRect().height - contentBoxNode.getBoundingClientRect().height) / 2;
                          ...
                          return (
                            <tr key={`row_${index}`}>
                              {[...Array(8)].map((sdata, sindex) => {
                                ...
                                return (
                                  <td key={`box_${cindex}`} id={"box_" + cindex} data-block={cindex} style={{ width: "100px", height: "100px", border: "1px solid white" }}>
                                    <img ref={"drag-" + cindex} onClick={() => this.verifySwapImages(currentIndex)} data-id={cindex} src={("./puzzles/" + cNum)} alt={cindex} />
                                  </td>
                                );
                              })}
                            </tr>
                        ...
                        return (
                          <tr key={`row2_${index}`}>
                            {[...Array(8)].map((sdata, sindex) => {
                              aindex = aindex + 1;
                              if (aindex === 40) {
                                return <td key={`box2_${aindex}`} id={"box_" + aindex} style={{ minWidth: "10px", minHeight: "10px", backgroundColor: "#FFF" }}>
                                  &nbsp;
                                </td>;
                              } else {
                                return <td key={`box2_${aindex}`} id={"box_" + aindex} style={{ minWidth: "10px", minHeight: "10px" }}>
                                  &nbsp;
                                </td>;
                              }
                            })}
